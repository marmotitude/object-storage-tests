...
import { aws, swift, swiftSetupCheck } from './utils.js'
...
export function setup() {
  ...
  testBuckets.forEach(bucketName => {
    const output = aws("s3", [ "mb", `s3://${bucketName}` ])
    console.log(output)
    check(output, {
...
export function teardown({testBuckets, largeFiles}) {
  testBuckets.forEach(bucketName => {
    const output = aws("s3", [ "rb", "--force", `s3://${bucketName}` ])
    console.log(output)
    check(output, {
...
export function uploadWithSegments({testBuckets, largeFiles}) {
  ...
  testBuckets.forEach((bucketName, i) => {
    ...
    const output = aws("s3", [ "cp", "--no-progress", largeFiles[i], `s3://${bucketName}/${objectName}` ])
    console.log(output)
    check(output, {
      "aws s3 cp success": o => o.includes(bucketName) && o.includes(objectName),
    })
    bucketStats.push(
      swift("stat", [`${bucketName}+segments`])
    )
  })
